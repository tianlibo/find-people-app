module V2
  class Players < Grape::API
    ...
    resource :players do
      ...
      post :create do 
        @player = Player.new  params[:player].as_json
        if @player.save
      ...
      put ':id' do 
        ...
        if Player.redis_update params[:id], params[:player].as_json
          #更新地图
          Map.redis_map_update(params[:player][:longitude],params[:player][:latitude],old_player)
          {code: 0, info: ""}
        else
          {code: 1, info: @user.errors.messages}
        end
